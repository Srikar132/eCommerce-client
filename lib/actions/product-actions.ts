            ...
            if (validatedData.variants.length > 0) {
                await db.insert(productVariants).values(
                    validatedData.variants.map((variant) => ({
                        productId: newProductId!,
                        // Cast to any to satisfy Drizzle's strict Enum types
...
// ============================================================================
// MAPPERS
// FIX: numeric → string type leak.
// All parseFloat calls live HERE and nowhere else. Every function that reads
// a numeric column must go through these mappers — callers always receive
// proper JS numbers, never raw Drizzle numeric strings.
// ============================================================================
...
function parseVariant(variant: typeof productVariants.$inferSelect): ProductVariant {
    return {
        ...
        size: variant.size,
        color: variant.color,
        colorHex: variant.colorHex,           // now non-nullable — always a string
        stockQuantity: variant.stockQuantity,
        priceModifier: parseFloat(variant.priceModifier),   // FIX: renamed from additionalPrice
        sku: variant.sku,
...
export async function getAllProducts(params: ProductParams): Promise<PagedResponse<Product>> {
    ...
    try {
        ...
        // 3. Variant Filters (Sizes and Colors)
        if (sizes || colors) {
            const variantConditions = [];
            
            if (sizes) {
                const sizeList = sizes.split(",").map(s => s.trim());
                variantConditions.push(inArray(productVariants.size, sizeList as (typeof productVariants.size.enumValues[number])[]));
            }
            
            if (colors) {
                const colorList = colors.split(",").map(c => c.trim());
                variantConditions.push(inArray(productVariants.color, colorList as (typeof productVariants.color.enumValues[number])[]));
            }
        ...
        let orderByClause;
        switch (sortBy) {
            case "NAME_ASC": orderByClause = asc(products.name); break;
            case "NAME_DESC": orderByClause = desc(products.name); break;
            case "PRICE_ASC": orderByClause = asc(products.basePrice); break;
            case "PRICE_DESC": orderByClause = desc(products.basePrice); break;
            case "CREATED_AT_ASC": orderByClause = asc(products.createdAt); break;
            default: orderByClause = desc(products.createdAt); break;
        }
        ...
        const variantsByProduct = variantsResults.reduce((acc, variant) => {
            (acc[variant.productId] ??= []).push(parseVariant(variant));
            return acc;
...
// ============================================================================
// GET PRODUCT BY SLUG
// FIX: was making 2 DB round-trips (product then images), and returning
// variants: [] hardcoded. Now fetches product + images + variants in parallel
// — 2 parallel queries instead of 2 serial queries, and variants are real.
// ============================================================================
...
export async function getProductBySlug(slug: string): Promise<Product | null> {
    try {
        ...
        // FIX: fetch images AND variants in parallel — single round-trip pair
        // instead of sequential calls. Variants are now populated, not [].
        const [imagesResults, variantsResults] = await Promise.all([
            db
                .select()
                .from(productImages)
        ...
        return {
            ...
            slug: product.slug,
            description: product.description ?? undefined,
            basePrice: parseFloat(product.basePrice),   // FIX: parsed at mapper
            sku: product.sku,
            material: product.material ?? undefined,
            careInstructions: product.careInstructions ?? undefined,
            isActive: product.isActive,
...
export async function getProductVariants(productId: string): Promise<ProductVariant[]> {
    try {
        ...
        // FIX: parseVariant mapper handles all numeric → number parsing
