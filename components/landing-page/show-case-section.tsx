...
export default async function ShowCaseSection() {
  ...
  return (
    <section className="relative section  w-full overflow-hidden min-h-screen lg:min-h-[110vh] py-16 sm:py-20 md:py-24">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 lg:gap-12 xl:gap-16 items-center min-h-[80vh]">
          ...
          <div className="flex flex-col items-center space-y-8 sm:space-y-10 lg:space-y-12">
            {showcaseProducts.slice(0, 2).map((product, index) => (
              <div
                key={product.id}
                className={`transform hover:scale-105 transition-all duration-500 ${index === 0 ? '-rotate-3 hover:rotate-0' : 'rotate-6 hover:rotate-0'
                  }`}
              >
